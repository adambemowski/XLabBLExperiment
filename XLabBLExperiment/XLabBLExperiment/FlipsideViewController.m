...
- (void) showLine {
    ...
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    NSPredicate *p = [NSPredicate predicateWithFormat:@"expId == %@",[defaults objectForKey:@"currentExpID"]];
    [formatter1 release];
    [fetchRequest setPredicate:p];
    
    //sort the results
    NSSortDescriptor *sessionSort = [[NSSortDescriptor alloc] initWithKey:@"sessionID" ascending:YES selector:nil];
    NSSortDescriptor *lineSort = [[NSSortDescriptor alloc] initWithKey:@"lineID" ascending:YES selector:nil];
    NSArray *sortDescriptors = [NSArray arrayWithObjects:sessionSort, lineSort, nil];
    ...
    line = [pollResults objectAtIndex:0];
    if (!pollResults) {
        //serious error restart the app
        NSLog(@"encountered a serious error please restart the app!");
    }
    
	entity = [NSEntityDescription entityForName:@"ExperimentBL" inManagedObjectContext:self.managedObjectContext];
	[fetchRequest setEntity:entity];
    
	//setpredicate to search for current experiment ID
    p = [NSPredicate predicateWithFormat:@"ID == %@",line.expId];
    [fetchRequest setPredicate:p];
    sortDescriptors = [NSArray arrayWithObjects: nil];
    [fetchRequest setSortDescriptors:sortDescriptors];
    ...
    experiment = [pollResults objectAtIndex:0];
    ...
    submitButton.hidden = NO;
    //set up budget lines
    FSexperiment = [line.expId intValue];
    FSsession = [line.sessionID intValue];
    FSline = [line.lineID intValue];
    NSLog(@"[FSVC] setupbudgetline");
    topNavBar.title = experiment.title;
    NSNumber *xMaxFromDic = experiment.x_max;
    NSNumber *yMaxFromDic = experiment.y_max;
    NSNumber *totalMax = MAX(xMaxFromDic, yMaxFromDic);
    xIntercept = line.x_int;
    yIntercept = line.y_int;
    xUnits = experiment.x_units;
    yUnits = experiment.y_units;
    NSLog(@"%@,%@",experiment.y_max,xMaxFromDic);
    xLength = [xIntercept floatValue]/[totalMax floatValue]*200;
    yLength = [yIntercept floatValue]/[totalMax floatValue]*200;
    BLG.xint = 50.0f + xLength;
    BLG.yint = 220.0f - yLength;
